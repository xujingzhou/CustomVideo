...
{
	long selectedIndex = [_items indexOfObject:selectedItem];
				  ...
				  menuDirection:(PCStackMenuDirection)direction
{
	...
	long max = MAX([titles count], [images count]);
	for(int i = 0; i < max; i++)
	{
		...
		PCStackMenuItem *stackItem = [[PCStackMenuItem alloc] initWithFrame:r withTitle:title withImage:image alignment:(direction == PCStackMenuDirectionClockWiseUp || direction == PCStackMenuDirectionCounterClockWiseDown) ? NSTextAlignmentRight : NSTextAlignmentLeft];
		stackItem.alpha = 0.5;
		stackItem.hidden = YES;
